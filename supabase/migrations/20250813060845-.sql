...
create policy if not exists "select own invoices" on public.invoices
for select using (auth.uid() = user_id);

create policy if not exists "insert own invoices" on public.invoices
for insert with check (auth.uid() = user_id);

create policy if not exists "update own invoices" on public.invoices
for update using (auth.uid() = user_id);

create policy if not exists "delete own invoices" on public.invoices
for delete using (auth.uid() = user_id);

-- Policies for invoice_items (scoped via parent invoice)
create policy if not exists "select own items" on public.invoice_items
for select using (
  exists (
...
create policy if not exists "insert own items" on public.invoice_items
...
create policy if not exists "update own items" on public.invoice_items
for update using (
  exists (
    select 1 from public.invoices inv where inv.id = invoice_id and inv.user_id = auth.uid()
  )
...
create policy if not exists "delete own items" on public.invoice_items
for delete using (
  exists (
    select 1 from public.invoices inv where inv.id = invoice_id and inv.user_id = auth.uid()
  )
